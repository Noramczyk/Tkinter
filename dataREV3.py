...
class logSuccess:
    ...
    # **********************************************************************
        def setFunc():
            ...
            threshVal = simpledialog.askinteger("Theshold Value ", "Enter new value: ")    # FINALLY !!!
            #plt.plot([0., Counter], [threshold, threshold], "k--")

            #line1, = window.ax.plot(x, y, 'b-')
                                                             # initial Threshold Set
            x_coord = [0,Counter]
            y_coord = [threshVal,threshVal]
            line1 = plt.plot(x_coord, y_coord)

            #line1 = plt.axhline(y = line1, color = 'b', linestyle = ':')
            #line1, = window.ax.plot(x, y, 'b-')

    # ******************************************************************
        def clear():
            plt.ion()
            line1 = line1.pop(0).remove()
            #line.remove()
